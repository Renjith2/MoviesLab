...
function WatchList() {
  ...
  useEffect(() => {
    let moviesfromStorage = localStorage.getItem("imdb");
    moviesfromStorage = JSON.parse(moviesfromStorage);
    setfavourites(moviesfromStorage);
  }, []);
  ...
  return (
    <>
      <div className="mt-6 flex space-2 justify-center">
        {genres.map((genre) => {
          return (
            <button
              className={
                currGenre == genre
                  ? "m-2 text-lg p-1 px-2 bg-blue-400 text-white rounded-x1 font-bold"
                  : "m-2 text-lg p-1 px-2 bg-gray-400 hover:bg-blue-400 text-white rounded-x1 font-bold"
              }
      ...
      <div className="text-center">
        <input type="text" className="border-4 text-center p-1 m-2 bg-gray-200"  placeholder="Search for Movies" value={searchStr} onChange={(e)=>setSearchStr(e.target.value)}></input>
      </div>
